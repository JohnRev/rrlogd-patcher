...
patterns = [b"\xF2\x04\x03\xFF\xF7\x45\xFF", b"\xFE\x33\x46\x4B\xA8\x10\x22"]
replaces = [b"\xF2\x04\x03\xE3\x20\x45\xFF", b"\xFE\x33\x46\x47\xA8\x10\x22"]
...
for i in range(len(patterns)):
   ...
   for match_obj in regex.finditer(data):
       ...
       print ("Original %08X: "%offset, ', '.join("%02X"%(ch) for ch in data[offset:offset+7]))
       print ("Patched  %08X: "%offset, ', '.join("%02X"%(ch) for ch in replace))
       data[offset:offset+7] = replace
